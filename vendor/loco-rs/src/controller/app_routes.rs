        ...
        self.get_routes()
            ...
            .flat_map(|controller| {
                ...
                controller.handlers.iter().map(move |handler| {
                    ...
                    // convert colon-style parameters ":id" into axum-compatible
                    // brace syntax "{id}" so the router doesn't panic at runtime
                    // when building routes (see issue with axum v0.8). this keeps
                    // backwards compatibility with older route definitions.
                    let uri = {
                        // simple regex to capture segment names after ':'
